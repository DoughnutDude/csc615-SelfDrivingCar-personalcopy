 ...
 */
static void PCA9685_SetPWM(int deviceNum, UBYTE channel, UWORD on, UWORD off)
{
    PCA9685_WriteByte(deviceNum, LED0_ON_L + 4*channel, on & 0xFF);
    PCA9685_WriteByte(deviceNum, LED0_ON_H + 4*channel, on >> 8);
    PCA9685_WriteByte(deviceNum, LED0_OFF_L + 4*channel, off & 0xFF);
    PCA9685_WriteByte(deviceNum, LED0_OFF_H + 4*channel, off >> 8);
}
 ...
 * PCA9685_Init(0x40);
 */
void PCA9685_Init(int deviceNum, char addr)
{
    DEV_I2C_Init(deviceNum, addr);
    I2C_Write_Byte(deviceNum, MODE1, 0x00);
}
 ...
 */
void PCA9685_SetPWMFreq(int deviceNum, UWORD freq)
{
    ...
    UBYTE oldmode = PCA9685_ReadByte(deviceNum, MODE1);
    UBYTE newmode = (oldmode & 0x7F) | 0x10; // sleep

    PCA9685_WriteByte(deviceNum, MODE1, newmode); // go to sleep
    PCA9685_WriteByte(deviceNum, PRESCALE, prescale); // set the prescaler
    PCA9685_WriteByte(deviceNum, MODE1, oldmode);
    DEV_Delay_ms(5);
    PCA9685_WriteByte(deviceNum, MODE1, oldmode | 0x80);  //  This sets the MODE1 register to turn on auto increment.
}
 ...
 */
void PCA9685_SetPwmDutyCycle(int deviceNum, UBYTE channel, UWORD pulse)
{
    PCA9685_SetPWM(deviceNum, channel, 0, pulse * (4096 / 100) - 1);
}
 ...
 */
void PCA9685_SetLevel(int deviceNum, UBYTE channel, UWORD value)
{
    if (value == 1)
        PCA9685_SetPWM(deviceNum, channel, 0, 4095);
    else
        PCA9685_SetPWM(deviceNum, channel, 0, 0);
}
