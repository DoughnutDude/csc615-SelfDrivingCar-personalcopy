...
// Specifically for when using sysfs.
void DEV_GPIO_Unexport(UWORD Pin) {
    DEBUG("unexporting pin: %d\r\n", Pin);
...
Info:
******************************************************************************/
void DEV_I2C_Init(int deviceNum, uint8_t Add)
{
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
        ...
        char i2c_device[11] = "";
        sprintf(i2c_device, "/dev/i2c-1", deviceNum+1);
        DEBUG("dev_i2c_init deviceNum: %s\r\n", deviceNumAsChar);*/

        DEV_HARDWARE_I2C_begin(deviceNum, "/dev/i2c-1");
        DEV_HARDWARE_I2C_setSlaveAddress(deviceNum, Add);
    #endif
#endif
}

void I2C_Write_Byte(int deviceNum, uint8_t Cmd, uint8_t value)
{
	int ref;
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
    ...
    #elif USE_DEV_LIB
        char wbuf[2]={Cmd, value};
        DEV_HARDWARE_I2C_write(deviceNum, wbuf, 2);

    #endif
#endif
}

int I2C_Read_Byte(int deviceNum, uint8_t Cmd)
{
	int ref;
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
    ...
    #elif USE_DEV_LIB
        char rbuf[2]={0};
        DEV_HARDWARE_I2C_read(deviceNum, Cmd, rbuf, 1);
        ref = rbuf[0];
    #endif
#endif
    return ref;
...
int I2C_Read_Word(int deviceNum, uint8_t Cmd)
{
	int ref;
#if DEV_I2C
    #ifdef USE_BCM2835_LIB
    ...
    #elif USE_DEV_LIB
        char rbuf[2] = {0};
        DEV_HARDWARE_I2C_read(deviceNum, Cmd, rbuf, 2);
        ref = rbuf[1]<<8 | rbuf[0];
    #endif
#endif
    return ref;
